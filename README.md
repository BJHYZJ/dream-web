...
The [long-search recording](https://bjhyzj.github.io/dream-web/simulation/#long-search-07) shows case 07 completing after 78.7 minutes of robot-action time. The complete timeline plays at 24×; separate grasp and placement excerpts play at 1×. The camera panels are accompanied by a semantic-memory heatmap reconstructed from all 2,552 original observations, with each update checked against the recorded voxel counts. This is one qualified completion among four selected follow-up cases and does not change the main 38/50 result. The `long-search-07` anchor is also referenced by the reviewer response.
